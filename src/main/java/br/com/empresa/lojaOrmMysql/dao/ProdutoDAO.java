...
public class ProdutoDAO {
	...
	public List<Produto> buscarPorNome(String nome) {
		return em.createQuery("SELECT p FROM Produto p WHERE p.nome = :nome", Produto.class).setParameter("nome", nome) // Named
																														// parameters
				.getResultList();
	}

	public List<Produto> buscarPorNome2(String nome) {
		return em.createQuery("SELECT p FROM Produto p WHERE p.nome = ?1", Produto.class).setParameter(1, nome) // parametro
																												// posicional
				.getResultList();
	}
	...
	public BigDecimal buscarPrecoDoProdutoComNome(String nome) {
		return em.createQuery("SELECT p.preco FROM Produto p WHERE p.nome = :nome", BigDecimal.class)
				.setParameter("nome", nome).getSingleResult(); // carrega um único resultado de uma consulta
	}
